(function ($) {
  ...
  if ($(".loadMoreItem").length > 0) {
    $(".loadMoreItem").on("click", function (e) {
      ...
      if (count < 3) {
        $.ajax({
          ...
          success: function (data) {
            setTimeout(function () {
              ...
              [].slice
                .call(document.querySelectorAll(".folio_effect > .folio_item"))
        ...
        $(Grid).append(
          '<div class="col-lg-12 alertCols"><div class="alert alert-warning folioAlert" role="alert">Sorry! No more posts available.</div></div>'
        );
        $this.removeClass("working");
  ...
  if ($("#map").length > 0) {
    ...
    if ($("body").hasClass("light")) {
      var styles = [
        { featureType: "road", stylers: [{ color: "#eaeaeb" }] },
        ...
        { featureType: "poi", stylers: [{ color: "#eceef7" }] },
        {
      ...
      var styles = [
        { featureType: "road", stylers: [{ color: "#292533" }] },
        { featureType: "water", stylers: [{ color: "#262330" }] },
        { featureType: "landscape", stylers: [{ color: "#312e3b" }] },
        { elementType: "labels.text.fill", stylers: [{ color: "#262330" }] },
        { featureType: "poi", stylers: [{ color: "#201e29" }] },
        {
          elementType: "labels.text",
          stylers: [{ saturation: 1 }, { weight: 0.1 }, { color: "#aaa9ac" }],
        },
      ];
    }
    map.addStyle({
  ...
  if ($("#contactForm").length > 0) {
    $("#contactForm").on("submit", function (e) {
      ...
      $('button[type="submit"]', $this).html(
        '<span><i class="icon icon-Restart"></i>Processing...</span>'
      );
      var required = 0;
      ...
      if (required === 0) {
        $.ajax({
          ...
          success: function (data) {
            ...
            $('button[type="submit"]', $this).html(
              '<span><i class="icon icon-Like"></i>Done!</span>'
            );
            setTimeout(function () {
              $('button[type="submit"]', $this).html(
                '<span><i class="icon icon-Mail"></i>Send Message</span>'
              );
            }, 1500);
          },
        });
        ...
        $('button[type="submit"]', $this).html(
          '<span><i class="icon icon-Dislike"></i>Failed!</span>'
        );
        setTimeout(function () {
          $('button[type="submit"]', $this).html(
            '<span><i class="icon icon-Mail"></i>Send Message</span>'
